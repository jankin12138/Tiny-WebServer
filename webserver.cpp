                     ...
                     int opt_linger, int trigmode, int sql_num, int thread_num, int close_log, int actor_model)
{
    ...
    m_OPT_LINGER = opt_linger;//初始优雅关闭连接
    m_TRIGMode = trigmode;//初始化触发组合模式
    m_close_log = close_log;    //关闭日志，默认0不关闭
    m_actormodel = actor_model;//初始化并发模式，默认proactor
}
...
void WebServer::eventLoop()
{
    ...
    while (!stop_server)
    {
        //等待所监控文件描述符上有事件的产生
        //检测发生事件的文件描述符
        int number = epoll_wait(m_epollfd, events, MAX_EVENT_NUMBER, -1);//返回最大事件
